...
function calculateResults() {
    // Calculate life score based on answers using direct year additions
    let totalYears = 0;
    
    currentUser.answers.forEach(answer => {
        totalYears += answer.weight; // Weights are now 0, 1, 2, or 3 years
    });
    
    // Base remaining years (minimum) + bonus years from questions
    const baseRemainingYears = 10; // Minimum years
    const bonusYears = totalYears; // Total bonus years from all questions
    const remainingYears = baseRemainingYears + bonusYears;
    
    // Ensure we stay within reasonable bounds (10-50 years)
    const finalRemainingYears = Math.min(Math.max(remainingYears, 10), 50);
    
    // Use actual age from user input
    const currentAge = currentUser.age;
    
    // Calculate total projected lifespan
    const projectedLifespan = currentAge + finalRemainingYears;
    
    // Calculate life wasted percentage
    const lifeWastedPercentage = (currentAge / projectedLifespan) * 100;
    
    currentUser.lifeScore = finalRemainingYears;
    currentUser.projectedLifespan = projectedLifespan;
    currentUser.lifeWastedPercentage = lifeWastedPercentage;
    
    // Show results
    showScreen('results');
    displayResults(finalRemainingYears);
}
